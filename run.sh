...
composer_update() {
	...
	run_cmd "exec server composer update"
}

composer_install() {
	echo "Installing composer..."
	run_cmd "exec server composer install"
}

bash_cmd() {
	echo "Running bash in container..."
	run_cmd "exec server bash"
}

mysql_cmd() {
	echo "Running mysql in container..."
	run_cmd "exec mysql mysql -uroot -h127.0.0.1 -P3306 -p${MYSQL_ROOT_PASSWORD}"
}
