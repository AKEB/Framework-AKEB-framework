...
docker run --rm -v "${HOST_DIR}":/opt -w /opt -e XDEBUG_MODE='debug' -e XDEBUG_CONFIG='client_host=host.docker.internal client_port=9003' akeb/nginx-php-fpm-${version}:latest /bin/bash -c "${CMD}"
