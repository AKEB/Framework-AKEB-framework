...
function vardump($v, $error_log=false, $html_encode=true) {
	...
	if ($error_log) {
		...
		if ($html_encode) $text = htmlspecialchars($text);
		print $text."\n";
		print "</pre>\n";
	}
}

function file_anticache($filename) {
	if (!$filename) return '';
	if (str_contains((string) $filename,'http')) {
		return $filename;
	}
	$fileTime = filemtime(constant('SERVER_ROOT').$filename);
	return $filename.(!str_contains((string) $filename,'?') ? '?': '&').'mt='.$fileTime;
...
function common_change_query($param, $value): string {
	$result = [];
...
function common_redirect(string $url='', array $param=[], bool $post=false) {
	global $error;
	$err = $error ? $error : ($_GET['error']??false);
	...
	if (!$post) {
		...
		$html = '<html><body>';
		$html .= '<script type="text/javascript" nonce="'.\CSP::nonceRandom().'">';
...
function common_assemble_flags(array|int $flags=0): int {
	$ret = 0;
	if (is_array($flags)) {
