...
class Common {
	...
	public static function save(\DB\Database &$db_obj, string $table_name, array $param, string $table_fields='', string $ref_name='id' , string $add=''): int|bool {
	...
	public static function delete(\DB\Database &$db_obj, string $table_name, mixed $ref=false, string $add='', string $ref_name='id') {
		...
		\Config::getInstance()->app_debug ?
			error_log(
				...
				$query)
			: null;
		// mrgsLogs_log('Delete '.($db_obj->database_master ? 'master
