...
class FixedBitNotation {
     ...
     * @param boolean $_rightPadFinalBits How to encode last character
     * @param boolean $_padFinalGroup Add padding to end of encoded
     *                output
    ...
    public function __construct(
		$bitsPerCharacter,
		$chars = NULL,
		protected $_rightPadFinalBits = FALSE,
		protected $_padFinalGroup = FALSE,
		$padCharacter = '='
	) {
					...
					if ($padFinalGroup) {
						...
						$pads = $bytesPerGroup * 8 / $bitsPerCharacter
							- ceil((strlen($rawString) % $bytesPerGroup)
								* 8 / $bitsPerCharacter);
						$encodedString .= str_repeat((string) $padCharacter[0], $pads);
					}
