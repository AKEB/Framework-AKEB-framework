		...
		*/
		...
		private function _toType($value) {
			...
			if ( is_numeric($value) && preg_match ('/^(-|)[1-9]+[0-9]*$/', $value) ){
				$int_value = (int)$value;
				if ($int_value != PHP_INT_MAX && $int_value != ~PHP_INT_MAX)
					$value = $int_value;
				return $value;
			}
		...
		private function _inlineEscape($inline) {
			...
			do {
				...
				// Check for sequences
				while (preg_match('/\[([^{}\[\]]+)\]/U',$inline,$match_seqs)) {
					$seqs[] = $match_seqs[0];
					$inline = preg_replace('/\[([^{}\[\]]+)\]/U', ('YAMLSeq' . (count($seqs) - 1) . 's'), $inline, 1);
				}

				// Check for mappings
				while (preg_match('/{([^\[\]{}]+)}/U',$inline,$match_maps)) {
					$maps[] = $match_maps[0];
				...
				if (!empty($seqs)) {
					foreach ($explode as $key => $value) {
						if (strpos($value,'YAMLSeq') !== false) {
							foreach ($seqs as $seq_k => $seq) {
								$explode[$key] = str_replace(('YAMLSeq'.$seq_k.'s'),$seq,$value);
								$value = $explode[$key];
							}
						}
					}
				...
				if (!empty($maps)) {
					foreach ($explode as $key => $value) {
						if (strpos($value,'YAMLMap') !== false) {
							foreach ($maps as $map_k => $map) {
								$explode[$key] = str_replace(('YAMLMap'.$map_k.'s'), $map, $value);
								$value = $explode[$key];
							}
						}
					}
				...
				if (!empty($saved_strings)) {
					foreach ($explode as $key => $value) {
						while (strpos($value,'YAMLString') !== false) {
							$explode[$key] = preg_replace('/YAMLString/',$saved_strings[$string_i],$value, 1);
							unset($saved_strings[$string_i]);
							++$string_i;
							$value = $explode[$key];
						}
					}
		...
		private function nodeContainsGroup ($line) {
			...
			if (preg_match('/(\*['.$symbolsForReference.']+$)/', $line, $matches)) return $matches[1];
			if (preg_match ('#^\s*<<\s*:\s*(\*[^\s]+).*$#', $line, $matches)) return $matches[1]; return false;
		}
